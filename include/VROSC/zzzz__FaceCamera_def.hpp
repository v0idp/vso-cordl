#pragma once
// IWYU pragma private; include "VROSC/FaceCamera.hpp"
#include "beatsaber-hook/shared/utils/typedefs.h"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include "UnityEngine/zzzz__MonoBehaviour_def.hpp"
CORDL_MODULE_EXPORT(FaceCamera)
// Forward declare root types
namespace VROSC {
class FaceCamera;
}
// Write type traits
MARK_REF_PTR_T(::VROSC::FaceCamera);
// Dependencies UnityEngine.MonoBehaviour
namespace VROSC {
// Is value type: false
// CS Name: VROSC.FaceCamera
class CORDL_TYPE FaceCamera : public ::UnityEngine::MonoBehaviour {
public:
// Declarations
static inline ::VROSC::FaceCamera* New_ctor() ;

/// @brief Method OnDrawGizmos, addr 0x173a020, size 0x2b4, virtual false, abstract: false, final false
inline void OnDrawGizmos() ;

/// @brief Method Update, addr 0x1739f68, size 0xb8, virtual false, abstract: false, final false
inline void Update() ;

/// @brief Method .ctor, addr 0x173a2d4, size 0x8, virtual false, abstract: false, final false
inline void _ctor() ;

protected:
// Ctor Parameters []
// @brief default ctor
constexpr FaceCamera() ;
public:

// Ctor Parameters [CppParam { name: "", ty: "FaceCamera", modifiers: "&&", def_value: None }]
// @brief delete move ctor to prevent accidental deref moves
FaceCamera(FaceCamera && ) = delete;

// Ctor Parameters [CppParam { name: "", ty: "FaceCamera", modifiers: "const&", def_value: None }]
// @brief delete copy ctor to prevent accidental deref copies
FaceCamera(FaceCamera const& ) = delete;

/// @brief IL2CPP Metadata Type Index
static constexpr uint32_t  __IL2CPP_TYPE_DEFINITION_INDEX{1093};

static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
static_assert(::cordl_internals::size_check_v<::VROSC::FaceCamera, 0x20>, "Size mismatch!");

} // namespace end def VROSC
NEED_NO_BOX(::VROSC::FaceCamera);
DEFINE_IL2CPP_ARG_TYPE(::VROSC::FaceCamera*, "VROSC", "FaceCamera");
